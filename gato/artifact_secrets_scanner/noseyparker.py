            ...
            if findings:
                for finding in findings:
                    if not self.include_all_artifact_secrets:
                        # added filters to remove common false positives; can remove to get more information if desired
                        if finding['rule_name'] == "PEM-Encoded Private Key" or finding['rule_name'] == "Base64-PEM-Encoded Private" or  finding['rule_name'] == "Generic Password":
                            continue
                    ...
                    for match in finding['matches']:
                        ...
                        if 'snippet' in match:
                            ...
                            # Excluding common false positives
                                exclusions = ["test", "TEST", "EXAMPLE", "example", "anon", "passwd", "pwd", "secretsmanager", "secret_config"]
                                ...
                                if (finding['rule_name'] == "Generic API Key" and ".js" in match["path"]) or "README.md" in match["path"]:
                                    add_finding = False
                                    continue
